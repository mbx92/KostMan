...
const form = reactive({
  ...
  paymentMethod: 'cash' as 'cash' | 'transfer' | 'credit_card' | 'debit_card' | 'e_wallet' | 'other',
...
const handleSubmit = async () => {
  if (form.amount <= 0 || form.amount > remainingAmount.value) {
    toast.add({
      title: 'Invalid Amount',
      description: `Amount must be between Rp 1 and ${formatCurrency(remainingAmount.value)}`,
      color: 'error',
    })
    return
  }
    ...
    <div class="space-y-4">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <UFormField label="Catatan" help="Opsional">
            <UTextarea v-model="form.notes" :rows="2" placeholder="Tambahkan catatan..." class="w-full" />
          </UFormField>
          ...
          <div class="flex justify-end gap-2 pt-2">
            ...
            <UButton
              ...
              color="primary"
              :loading="submitting"
              :disabled="form.amount <= 0 || form.amount > remainingAmount"
            >
              Catat Pembayaran
            </UButton>
          </div>
